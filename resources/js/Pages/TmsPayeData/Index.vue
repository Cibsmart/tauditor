        ...
        <TableBody v-for="payroll in payrolls.data" :key="payroll.id">
          ...
          <TableRow v-if="show_detail[payroll.id]">
            <TableCell colspan="6">
              <Table>
                <TableBody>
                  <TableRow v-for="category in payroll.categories" v-show="category.payment_type_id==='sal'"
                            :key="category.id">
                    <TableCell class="">
                    ...
                    <TableCell class="text-right ">
                      <Link v-show="!category.uploaded" :href="route('paye.upload', {category: category.id})"
                            class="px-5 py-3" preserve-scroll preserve-state>Upload
                      </Link>

                      <Link v-show="category.uploaded" :href="route('paye.upload', {category: category.id})"
                            class="px-5 py-3" preserve-scroll preserve-state>Re-Upload
                      </Link>
